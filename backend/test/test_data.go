...
func InjectTestData(conn *pgx.Conn) error {

    // Insert test data into the users table
    var user1ID, user2ID int
    err := conn.QueryRow(context.Background(), "INSERT INTO users (username, password) VALUES ($1, $2) RETURNING id", "user1", "pass1").Scan(&user1ID)
    if err != nil {
        return fmt.Errorf("failed to insert user1: %v", err)
    }
    err = conn.QueryRow(context.Background(), "INSERT INTO users (username, password) VALUES ($1, $2) RETURNING id", "user2", "pass2").Scan(&user2ID)
    if err != nil {
    ...
    for _, data := range testData {
        insertSQL := `
            ...
            VALUES ($1, $2, $3)
        `
        _, err := conn.Exec(context.Background(), insertSQL, data.SensorID, data.Value, data.Time)
        if err != nil {
